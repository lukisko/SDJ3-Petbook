...
namespace business_logic.Model.Mediator
{
    public class Tier2User
    {
        ...
        /*public async Task<AuthorisedUser> GetUserByEmail(AuthorisedUser user){//TODO
            Console.WriteLine("name before serialize: "+ user.name);
            Comunication<AuthorisedUser> commClass = new Comunication<AuthorisedUser>("user","GetAllOf",user);

            Comunication<AuthorisedUser> theUser = await tier2.requestServerAsync<Comunication<AuthorisedUser>,Comunication<IList<Pet>>>(commClass);
